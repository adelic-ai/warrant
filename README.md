...
| Agent identity | **SPIFFE-shaped, not real SPIRE.** Short-lived X.509 certs with a `spiffe://warrant.local/agent/<id>` URI SAN, signed by a local CA generated fresh per process. `/identity/issue` is gated on a pre-shared per-agent bootstrap token (`WARRANT_BOOTSTRAP_TOKENS`) — the same join-token fallback real SPIRE itself offers absent a platform attestation plugin — so an arbitrary caller can no longer mint an identity for an arbitrary `agent_id`. What real SPIRE would still add: actual platform attestation (verifying what's running, not just checking a shared secret) and a persistent trust bundle — this has neither; disclosed in `warrant/identity.py`'s docstring. |
