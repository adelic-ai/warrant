# Demo — a real Strands-orchestrated run against a live `sage`
...
making real tool calls against a live `sage` instance, over real HTTP. The point is to replace a
synthetic delegation-chain test with agents actually doing something and being genuinely gated.

## What it does

`run_demo.py`:
1. Seeds a fresh `sage` database with the standard demo scenario and starts a real `uvicorn` server.
...
8. The harness saves the full agent transcript and `sage`'s own audit trail to `transcripts/`.
...
access to. The bug was real, in `sage/gateway.py`, not in the demo — the gateway was checking
...
- `sage_audit_log.json` — sage's own audit trail from that run.
